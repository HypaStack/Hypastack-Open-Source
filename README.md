...
**Three sidecars.** The Node app is fully self-sufficient; these take over specific jobs when they're up, and Node quietly does the work in-process when they're not:
...
- [`services/hypasan`](./services/hypasan) (Go) — user-input sanitization. Mirrors the Node pipeline for file notes (strip all HTML via bluemonday, scrub injection patterns, clamp length) so the app never has to load DOMPurify's jsdom dependency; the in-process version survives only as a lazy-loaded fallback for when the socket is down.
